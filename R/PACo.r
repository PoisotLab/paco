...
#' @param D A list of class \code{paco} as returned by \code{paco::add_pcoord} which includes Principle Coordinates for both phylogenetic distance matrices.
...
#' @return A \code{paco} object that now includes (alongside the Principle Coordinates and input distance matrices) the PACo sum of sqaured residuals, a p-value for this statistic, and the PACo statistics for each randomisation of the network if shuffled=TRUE in the PACo call.
